...
import name.zjq.blog.pcd.utils.UserAgentUtil;
...
public class User {
    ...
    public synchronized String createToken(HttpServletRequest request) {
        ...
        String token = new StringBuilder().append(username).append(UserAgentUtil.getIP(request)).append(UserAgentUtil.getUserAgentID(request))
                .append(uuid).toString();
        int x = (int) (Math.random() * 100);
        token = CoderUtil.MD5(token, x);
        md5times = x;
    ...
    public static User checkToken(String _token, HttpServletRequest request) {
        if (userlist.containsKey(_token)) {
            ...
            String token = new StringBuilder().append(u.username).append(UserAgentUtil.getIP(request)).append(UserAgentUtil.getUserAgentID(request))
